...
export class OllamaProvider extends AIProvider {
    ...
    async generateStream(prompt, options = {}, onChunk) {
        ...
        try {
            ...
            if (!response.ok) throw new LLMError(`Ollama respondió con status ${response.status}`);
            if (!response.body) throw new LLMError('No se recibió respuesta de Ollama');
            ...
            if (error.name === 'AbortError') throw new LLMError(`Ollama timeout después de ${timeout}ms`);
            if (error instanceof LLMError) throw error;
            logger.error({ err: error }, 'Error en streaming con Ollama');
            throw new LLMError(`Error al generar respuesta: ${error.message}`);
        } finally {
            clearTimeout(timer);
        }
